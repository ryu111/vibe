...
**每完成一個 task，必須立即自我檢查再繼續下一個 task**：

1. **命名一致性**：變數/函式名稱是否符合專案慣例（camelCase/snake_case）？
2. **邊界條件**：null/undefined 輸入是否有防護？陣列空值是否處理？
3. **錯誤處理**：是否有具體的錯誤處理（非 catch-all 吞錯誤）？
4. **介面對齊**：回傳值和型別是否符合 design.md 的介面定義？

⛔ **不允許跳過自檢繼續下一個 task。** 若發現問題，在當前 task 修正後再繼續。
