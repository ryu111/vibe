...
let passed = 0;
let failed = 0;
const { cleanTestStateFiles, writeTestState } = require('./test-helpers');
cleanTestStateFiles();
...
test('多個階段完成後各自有 tag', () => {
  ...
  // PLAN 完成
  let statePath = writeTestState(sessionId, {
    stages: ['PLAN', 'ARCH', 'DEV'],
    active: 'PLAN',
    pipelineId: 'standard',
    taskType: 'feature',
  ...
  try {
    ...
    statePath = writeTestState(sessionId, {
      stages: ['PLAN', 'ARCH', 'DEV'],
      completed: ['PLAN'],
      active: 'ARCH',
      pipelineId: 'standard',
...
// 注意：使用 writeTestState 建立 DAG 結構。DOCS 是最後階段，
