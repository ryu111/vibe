...
const CSS = `
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .flow-step { font-size: 0.78rem; padding: 0.15rem 0.55rem; border-radius: 4px; background: rgba(192,202,245,0.05); }
  ...
  .badge-auto { background: rgba(158,206,106,0.15); color: var(--green); }
  .badge-manual { background: rgba(122,162,247,0.15); color: var(--accent); }
  ...
  .status-complete { background: rgba(158,206,106,0.15); color: var(--green); }
  .status-in-progress { background: rgba(224,175,104,0.15); color: var(--yellow); }
  .status-planned { background: rgba(86,95,137,0.12); color: var(--text-muted); }

  /* 組件格 */
  .comp-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 0.5rem; margin-bottom: 0.8rem; }
  .comp-cell { text-align: center; padding: 0.5rem 0.2rem; border-radius: 6px; background: rgba(192,202,245,0.03); }
  ...
  .tag-skill { background: rgba(122,162,247,0.1); color: var(--accent); }
  .tag-agent { background: rgba(187,154,247,0.1); color: var(--purple); }
  .tag-hook { background: rgba(224,175,104,0.1); color: var(--yellow); }
  ...
  .agent-perm.readonly { background: rgba(122,162,247,0.12); color: var(--accent); }
  .agent-perm.writable { background: rgba(247,118,142,0.12); color: var(--red); }
  .agent-trigger { font-size: 0.78rem; color: var(--text-muted); display: flex; align-items: center; gap: 0.3rem; }
  .agent-trigger code { background: rgba(192,202,245,0.06); padding: 0.1rem 0.4rem; border-radius: 3px; font-size: 0.75rem; color: var(--accent); }
  .agent-flow { display: flex; align-items: center; gap: 0.3rem; flex-wrap: wrap; font-size: 0.72rem; color: var(--text-muted); }
  .agent-flow-step { padding: 0.1rem 0.4rem; border-radius: 3px; background: rgba(192,202,245,0.04); }
  .agent-flow .arrow { color: var(--text-muted); opacity: 0.5; }
  .agent-tools { display: flex; flex-wrap: wrap; gap: 0.2rem; }
  .agent-tool { font-size: 0.65rem; padding: 0.08rem 0.35rem; border-radius: 3px; background: rgba(192,202,245,0.05); color: var(--text-muted); }
  ...
  .agent-connector-label { font-size: 0.75rem; padding: 0.15rem 0.7rem; border-radius: 4px; background: rgba(192,202,245,0.04); border: 1px dashed var(--border); }
  .agent-human { width: 100%; max-width: 900px; margin: 0.3rem 0; padding: 0.7rem 1rem; border: 2px solid var(--yellow); border-radius: var(--radius); background: rgba(224,175,104,0.06); display: flex; align-items: center; gap: 0.8rem; }
  ...
  .pipe-agent { border: 1px solid var(--border); border-radius: var(--radius); padding: 0.8rem; background: rgba(192,202,245,0.02); }
  ...
  /* Main Agent */
  .pipe-main-agent { width: 100%; max-width: 780px; margin: 0 auto 0.5rem; display: flex; gap: 1rem; align-items: center; border: 2px solid var(--accent); border-radius: var(--radius); padding: 0.8rem 1.2rem; background: rgba(122,162,247,0.06); box-shadow: var(--card-shadow); }
  ...
  .guard-card.guide { border: 2px solid var(--cyan); background: rgba(125,207,255,0.06); }
  .guard-card.block { border: 2px solid var(--red); background: rgba(247,118,142,0.06); }
  ...
  .guard-hook code { background: rgba(192,202,245,0.06); padding: 0.08rem 0.35rem; border-radius: 3px; font-size: 0.7rem; }
  ...
  .guard-card.guide .guard-mechanism { background: rgba(125,207,255,0.15); color: var(--cyan); }
  .guard-card.block .guard-mechanism { background: rgba(247,118,142,0.15); color: var(--red); }
...
const colorToRgba = {
  ...
  // 故意使用 --text 色系（192,202,245 = #c0caf5）以獲得較亮的半透明底色，
  // 而非 --text-muted（86,95,137 = #565f89）的暗色基底
...
function genFlowDiagram(config, meta) {
  ...
  // 起點
  parts.push(`
    <div class="agent-human" style="border-color:var(--text-muted);background:rgba(192,202,245,0.02)">
  ...
  parts.push(`
    <div class="agent-human" style="border-color:var(--purple);background:rgba(187,154,247,0.06)">
  ...
  phases.forEach((phase, i) => {
    ...
    if (insertLayer) {
      const bg = colorToRgba[insertLayer.color] || 'rgba(192,202,245,0.02)';
      parts.push(`
    <div class="agent-connector">
  ...
  parts.push(`
    <div class="agent-connector"><div class="agent-connector-arrow">▼</div></div>
    <div class="agent-human" style="border-color:var(--green);background:rgba(158,206,106,0.06)">
      <div class="agent-human-icon">🚀</div>
      <div>
  ...
  bottomLayers.forEach((layer, idx) => {
    const bg = colorToRgba[layer.color] || 'rgba(192,202,245,0.02)';
...
function genAgentDetails(config, meta) {
  ...
  parts.push(`<div style="border:1px dashed var(--purple);border-radius:var(--radius);padding:0.6rem 0.8rem;background:rgba(187,154,247,0.04)">
  ...
  parts.push(`<div class="pipe-connector"><div class="pipe-connector-arrow">▼</div></div>`);
  parts.push(`<div class="pipe-main-agent" style="border-color:var(--green);background:rgba(158,206,106,0.06)">
...
function genColorPalette() {
  const colors = [
    ...
    { name: 'bg', hex: '#1a1b26' },
    { name: 'surface', hex: '#24283b' },
    { name: 'border', hex: '#3b4261' },
  ];
